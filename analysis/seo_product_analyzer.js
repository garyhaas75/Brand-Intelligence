...
function getCategoryPromptSection(specificType, categoryGroup, metaCache) {
  ...
  const geoFields = `${geoStartNum}. why_it_works: REQUIRED. JSON object. {"headline": "one sentence — the main reason to buy this product", "reasons": ["specific reason 1 — construction or design detail", "specific reason 2 — occasion or lifestyle fit", "specific reason 3 — wardrobe logic or versatility"]}. Reasons must be specific to THIS product. No generic statements like "great quality" or "versatile". Never null.
${geoStartNum + 1}. compatibility: REQUIRED. JSON object. {"pairs_with": [{"item": "product type name", "relationship_type": "often_bought_with"}, ...], "outfit_contexts": ["Named occasion 1", "Named occasion 2"]}. relationship_type must be exactly one of: often_bought_with, accessory, layering_piece. 2-4 pairs_with entries. 2-3 outfit_contexts as named moments (e.g. "Client lunch", not "Casual wear"). Never null.
${geoStartNum + 2}. care_notes: JSON object. {"fabric": "material composition", "washing": "wash instruction", "drying": "drying instruction", "ironing": "ironing note or null", "special_notes": "any care nuance or null"}. If explicit care instructions are not stated in the description, use standard care guidance for the fabric type visible or described (e.g. wool → "Dry clean or hand wash cold"). Only return null if truly no information is available AND fabric cannot be identified.
${geoStartNum + 3}. fit_logic: REQUIRED. JSON object. {"silhouette": "fit description from image — be specific about cut, structure, length", "best_for": "body type, height, or styling context this works best for", "sizing": "Runs true to size (if stated), or omit the sizing key if unknown", "styling_tip": "one concrete, specific tip for wearing this product"}. Use the image and product name to fill all fields. Never null.
${geoStartNum + 4}. customer_qa: REQUIRED. JSON array of exactly 3 objects. [{"question": "real customer question", "answer": "specific helpful answer"}, ...]. Always generate all 3 regardless of available information — use what's visible in the image and inferable from the product type. Questions should cover: fit/sizing, fabric/material, and occasion/styling. Never null.
${geoStartNum + 5}. use_cases: REQUIRED. JSON array of strings. Named moments this product is built for. Be specific: "Tuesday board meeting" not "work". "Weekend farmers market" not "casual". 3-5 items. Never null.`;
