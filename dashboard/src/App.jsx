    ...
    {emailPreviewItem && (() => {
      ...
      return (
        <div onClick={() => setEmailPreviewItem(null)}
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: '24px 16px' }}>
          <div onClick={e => e.stopPropagation()}
            style={{ width: '100%', maxWidth: 620, background: '#f4f4f4', borderRadius: 12, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
            ...
            <div style={{ background: '#fff', fontFamily: 'Arial, Helvetica, sans-serif' }}>
              ...
              <div style={{ background: '#f9f9f9', borderBottom: '1px solid #e5e5e5', padding: '10px 20px', textAlign: 'center' }}>
                {['NEW', 'BEST SELLERS', 'CLOTHING', 'SHOES', 'WATCHES'].map((nav, i) => (
                  <span key={nav}>
                    <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', color: '#333', cursor: 'default' }}>{nav}</span>
                    {i < 4 && <span style={{ color: '#ccc', margin: '0 8px' }}>|</span>}
                  </span>
                ))}
              </div>
              ...
              <div style={{ background: heroBg, padding: '48px 32px', textAlign: 'center' }}>
                ...
                <div style={{ display: 'inline-block', background: '#fff', color: '#000', fontWeight: 800, fontSize: 13, letterSpacing: '0.1em', padding: '12px 32px', cursor: 'default' }}>
                  {em.cta || 'SHOP NOW'}
                </div>
              </div>
                    ...
                    {products.map(p => {
                      ...
                      return (
                        <div key={p.handle} style={{ textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: 13, fontWeight: 600, color: '#111', marginBottom: 2, lineHeight: 1.3 }}>{p.name}</div>
                          <div style={{ fontSize: 13, color: '#666', marginBottom: 10 }}>${p.price}</div>
                          <div style={{ display: 'inline-block', border: '1px solid #111', color: '#111', fontWeight: 700, fontSize: 10, letterSpacing: '0.1em', padding: '7px 18px', cursor: 'default' }}>
                            SHOP NOW
                          </div>
                        </div>
                      )
                ...
                <div style={{ padding: '0 32px 32px', borderTop: products.length > 0 ? '1px solid #f0f0f0' : 'none' }}>
                  ...
                  <div style={{ background: '#fafafa', borderRadius: 4, padding: '16px 20px', border: '1px solid #eee' }}>
                    {em.bodyOutline.split(/\n|•/).filter(s => s.trim()).map((section, i) => (
                      <div key={i} style={{ display: 'flex', gap: 10, marginBottom: i < em.bodyOutline.split(/\n|•/).filter(s => s.trim()).length - 1 ? 10 : 0 }}>
                        <span style={{ color: '#000', fontWeight: 900, fontSize: 14, flexShrink: 0, marginTop: 1 }}>—</span>
                        <span style={{ fontSize: 13, color: '#444', lineHeight: 1.5 }}>{section.trim()}</span>
                      </div>
                    ))}
                  </div>
              ...
              {/* Primary CTA */}
              <div style={{ padding: '24px 32px 40px', textAlign: 'center' }}>
                <div style={{ display: 'inline-block', background: '#000', color: '#fff', fontWeight: 800, fontSize: 13, letterSpacing: '0.12em', padding: '14px 48px', cursor: 'default', width: '100%', boxSizing: 'border-box' }}>
                  {em.cta || 'SHOP NOW'}
                </div>
                {em.sendDay && (
                  <div style={{ fontSize: 11, color: '#999', marginTop: 10 }}>Recommended send day: {em.sendDay}</div>
                )}
              </div>
              ...
              <div style={{ background: '#f9f9f9', borderTop: '1px solid #e5e5e5', padding: '20px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 10, color: '#aaa', lineHeight: 1.6 }}>
                  530 Fifth Avenue, 12th Floor, New York, NY 10036<br />
                  customercare@anneklein.com · 1-844-390-AKNY<br />
                  <span style={{ textDecoration: 'underline', cursor: 'default' }}>Unsubscribe</span> · <span style={{ textDecoration: 'underline', cursor: 'default' }}>View in browser</span>
                </div>
              </div>
            </div>
          </div>
